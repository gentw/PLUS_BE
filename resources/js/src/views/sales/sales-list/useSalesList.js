...
export default function useUsersList() {
  ...
  const tableColumns = reactive([
    { key: 'client_name', sortable: true, label: 'Klienti' },
    { key: 'service_name', sortable: true, label: 'Sherbimi' },
  ...
  watch([currentPage, perPage, statusFilter, rangePicker, searchQuery], () => {
    ...
    if(statusFilter.value == 2 || statusFilter.value === null) {
      ...
      tableColumns.push(
        ...
        { key: 'created_at', sortable: true, label: 'Dt procesimit' },
        { key: 'updated_at', sortable: true, label: 'Dt Anulimit' },
        { key: 'actions' }
      );
    } else {
      tableColumns.splice(0, tableColumns.length);
  ...
  const deleteSale = (id) => {
    store.dispatch('app-sale/deleteSale', id).then(() => {
      refetchData() 
    });
  }
